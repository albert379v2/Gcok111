...
- Proxy Webshare SOCKS5 rotativo soportado
- Créditos en modo no-op (Opción 2) - siempre permite
...
GENERATOR_API_KEY = os.getenv('GENERATOR_API_KEY', '')  # API key para proteger /generate
FIVESIM_API_KEY = os.getenv('FIVESIM_API_KEY', '')
SERVICE_API_KEY = os.getenv('SERVICE_API_KEY', '')
API_BASE_URL = os.getenv('API_BASE_URL', '')

# Grizzly SMS
GRIZZLY_SMS_API_KEY = os.getenv('GRIZZLY_SMS_API_KEY', '')
GRIZZLY_SMS_BASE_URL = os.getenv('GRIZZLY_SMS_BASE_URL', 'https://api.grizzlysms.com')
...
# -------------------------------------------------------------------
# CONFIGURACIÓN DE PROXY (soporta HTTP, HTTPS, SOCKS5)
# -------------------------------------------------------------------
PROXY_AUTH = None
PROXY_HOST_PORT = None
PROXY_PROTOCOL = 'http'  # default

if PROXY_STRING:
    proxy_clean = PROXY_STRING.strip()

    # Detectar protocolo
    if proxy_clean.startswith('socks5://'):
        PROXY_PROTOCOL = 'socks5'
        proxy_clean = proxy_clean[9:]  # quitar socks5://
    elif proxy_clean.startswith('http://'):
        PROXY_PROTOCOL = 'http'
        proxy_clean = proxy_clean[7:]
    elif proxy_clean.startswith('https://'):
        PROXY_PROTOCOL = 'https'
        proxy_clean = proxy_clean[8:]

    if '@' in proxy_clean:
        PROXY_AUTH, PROXY_HOST_PORT = proxy_clean.split('@', 1)
    else:
...
# -------------------------------------------------------------------
# GRIZZLY SMS CONFIGURACIÓN
# -------------------------------------------------------------------
GRIZZLY_COUNTRY_MAP = {
    'BR': 73,    'UA': 1,     'GB': 16,    'US': 12,   # US Virtual (cheaper at $0.04)   'IN': 22,
    'ID': 6,     'PH': 7,     'KE': 48,    'TZ': 50,    'NG': 52,
    'PK': 54,    'BD': 56,    'VN': 58,    'KH': 60,    'LA': 62,
    'MM': 64,    'KG': 11,    'KZ': 2,     'RU': 0,     'RO': 32,
    'MD': 34,    'PL': 15,    'GE': 128,   'AZ': 35,    'AM': 33,
    'UZ': 40,    'TJ': 143,   'TM': 41,    'LV': 10,    'LT': 44,
    'EE': 34,    'CO': 52,    'MX': 54,    'PE': 56,    'CL': 58,
    'AR': 60,    'VE': 62,    'EC': 64,    'BO': 66,    'PY': 68,
    'UY': 70,    'MA': 37,    'CM': 41,    'GH': 43,    'CI': 45,
    'SN': 47,    'ML': 49,    'BF': 51,    'NE': 53,    'TG': 55,
    'BJ': 57,    'LR': 59,    'SL': 61,    'GN': 63,    'GW': 65,
    'CV': 67,    'GM': 69,
}

GRIZZLY_COUNTRY_ORDER = ['US', 'KG', 'KZ', 'UA', 'RU', 'RO', 'MD', 'GE', 'AZ', 'AM', 'UZ', 
                         'TJ', 'TM', 'LV', 'LT', 'EE', 'ID', 'PH', 'KE', 'TZ', 'NG',
                         'PK', 'BD', 'VN', 'KH', 'LA', 'MM', 'CO', 'PE', 'MX', 'CL',
                         'MA', 'CM', 'GH', 'CI', 'SN', 'ML', 'BF', 'NE', 'TG', 'BJ']
...
# ===================================================================
# OPCIÓN 2: CRÉDITOS EN MODO NO-OP (sin sistema de créditos externo)
# ===================================================================
...
# ===================================================================
# PROXY TESTING (con soporte SOCKS5)
# ===================================================================
...
def test_proxy(proxy_url, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            response = session.get('https://ipv4.webshare.io/', timeout=15)
...
# -------------------------------------------------------------------
# GRIZZLY SMS FUNCIONES
# -------------------------------------------------------------------

async def get_grizzly_prices():
    """Obtiene precios de Grizzly SMS para el servicio 'am' (Amazon)."""
    if not GRIZZLY_SMS_API_KEY:
        return {}
    base_url = GRIZZLY_SMS_BASE_URL.strip()
    ...
    url = f"{base_url}/stubs/handler_api.php"

    params = {
        'api_key': GRIZZLY_SMS_API_KEY,
        'action': 'getPrices',
        'service': 'am'
    }
    try:
        ...
        response = await loop.run_in_executor(None, lambda: requests.get(url, params=params, timeout=10))
        if response.status_code != 200:
            logger.warning(f"⚠️ No se pudo obtener precios de Grizzly: {response.status_code}")
            return {}
        
        data = response.json()
        prices = {}
        for country_code_str, services in data.items():
            if 'am' in services:
                info = services['am']
                cost = info.get('cost')
                count = info.get('count', 0)
                if cost is not None and count > 0:
                    iso_code = None
                    for iso, num in GRIZZLY_COUNTRY_MAP.items():
                        if str(num) == str(country_code_str):
                            iso_code = iso
                            break
                    if iso_code:
                        prices[iso_code] = float(cost)
                    else:
                        logger.debug(f"⚠️ País Grizzly ID {country_code_str} no mapeado a ISO")
        
        sorted_prices = sorted(prices.items(), key=lambda x: x[1])
        logger.debug(f"📊 Grizzly precios ordenados: {sorted_prices}")
        return dict(sorted_prices)
    except Exception as e:
        logger.warning(f"⚠️ Error obteniendo precios de Grizzly: {e}")
        return {}

async def get_grizzly_number(country_code, service='am'):
    """Compra un número de Grizzly SMS."""
    if not GRIZZLY_SMS_API_KEY:
        logger.warning("⚠️ No hay API key de Grizzly SMS")
        return None
    
    country_num = GRIZZLY_COUNTRY_MAP.get(country_code)
    if country_num is None:
        logger.error(f"❌ No hay mapeo de país Grizzly para {country_code}")
        return None
    ...
    try:
        loop = asyncio.get_running_loop()
        response = await loop.run_in_executor(None, lambda: requests.get(url, params=params, timeout=30))
        logger.debug(f"📡 Grizzly respuesta HTTP {response.status_code}")
        
        try:
            data = response.json()
        except ValueError:
            error_text = response.text.strip()
            logger.warning(f"Grizzly respuesta no JSON: {error_text}")
            if error_text == 'NO_NUMBERS':
                logger.warning("Grizzly: No hay números disponibles")
            elif error_text == 'BAD_KEY':
                logger.error("Grizzly: API key inválida")
            elif error_text == 'NO_BALANCE':
                logger.error("Grizzly: Saldo insuficiente")
            elif error_text == 'SERVICE_UNAVAILABLE_REGION':
                logger.warning("Grizzly: Servicio no disponible en tu región")
            return None
        ...
        if 'activationId' in data and 'phoneNumber' in data:
            ...
            logger.debug(f"📱 Número Grizzly comprado: {phone} (order_id: {order_id}, costo: {cost})")
            return phone, order_id
        else:
            logger.warning(f"⚠️ Respuesta inesperada de Grizzly: {data}")
            return None
            
    except Exception as e:
        logger.warning(f"⚠️ Error comprando número Grizzly: {e}")
        return None

async def get_grizzly_code(order_id, timeout=180):
    """Espera y obtiene el código SMS de Grizzly."""
    base_url = GRIZZLY_SMS_BASE_URL.strip()
            ...
            if response.status_code == 200:
                try:
                    ...
                    if text == 'STATUS_WAIT_CODE':
                        ...
                        code = text.split('STATUS_OK:')[1].strip()
                        logger.debug(f"📱 Código SMS recibido de Grizzly: {code}")
                        return code
                    else:
                        logger.debug(f"Grizzly status: {text}")
                        await asyncio.sleep(5)
                        continue
                ...
                if status == 'RECEIVED':
                    ...
                    if sms_list:
                        ...
                        codes = re.findall(r'\b(\d{5,6})\b', text)
                        if codes:
                            logger.debug(f"📱 Código SMS extraído de texto Grizzly: {codes[0]}")
                            return codes[0]
                elif status == 'PENDING':
                    pass
                else:
                    logger.warning(f"⚠️ Estado inesperado de Grizzly: {status}")
            
            await asyncio.sleep(5)
        except Exception as e:
            logger.debug(f"Grizzly waiting error: {e}")
            await asyncio.sleep(5)
    
    return None

async def cancel_grizzly(order_id):
    """Cancela una activación de Grizzly SMS (status=8) para reembolso."""
    if not GRIZZLY_SMS_API_KEY:
        return False
    base_url = GRIZZLY_SMS_BASE_URL.strip()
    ...
    try:
        ...
        if response.status_code == 200:
            text = response.text.strip()
            if text in ['ACCESS_CANCEL', 'ACCESS_CANCEL_ALREADY']:
                logger.debug(f"📱 Grizzly: activación {order_id} cancelada")
                return True
            else:
                logger.warning(f"⚠️ Grizzly cancel respuesta: {text}")
                return False
        else:
            logger.warning(f"⚠️ Grizzly cancel falló: {response.status_code}")
            return False
    except Exception as e:
        logger.warning(f"⚠️ Error cancelando Grizzly: {e}")
        return False

SMS_SERVICES = [
    {'name': 'grizzly', 'enabled': bool(GRIZZLY_SMS_API_KEY), 'get_number': get_grizzly_number, 'get_code': get_grizzly_code},
...
async def get_phone_number(account_country, force_service=None, force_country=None):
    """
    Obtiene un número de teléfono del servicio más barato disponible.
    ...
    De lo contrario, selecciona automáticamente el servicio y país más baratos.
    """
    ...
    # -------------------------------------------------------------------
    # OBTENER PRECIOS DE TODOS LOS SERVICIOS HABILITADOS
    # -------------------------------------------------------------------
    all_prices = []  # lista de (precio, servicio, país)

    # Grizzly prices
    grizzly_prices = await get_grizzly_prices()
    for iso, price in grizzly_prices.items():
        all_prices.append((price, 'grizzly', iso))

    # 5sim prices
    fivesim_prices = await get_fivesim_prices()
    for iso, price in fivesim_prices.items():
        all_prices.append((price, '5sim', iso))

    # Hero SMS no tiene API de precios, usamos orden fijo con precio estimado
    hero_estimated_prices = {
        'CM': 0.08, 'BR': 0.10, 'ID': 0.09, 'MA': 0.08, 'KG': 0.06, 'CO': 0.10, 'MX': 0.15
    }
    for iso in HERO_COUNTRY_ORDER:
        price = hero_estimated_prices.get(iso, 0.15)
        all_prices.append((price, 'hero', iso))

    # Ordenar todos por precio ascendente
    all_prices.sort(key=lambda x: x[0])

    # Priorizar números US si están en rango de precio aceptable (≤$0.10)
    us_entries = [(p, s, c) for p, s, c in all_prices if c == 'US' and p <= 0.10]
    if us_entries:
        # Mover US al principio si el precio es competitivo
        us_price, us_service, us_country = us_entries[0]
        all_prices = [(p, s, c) for p, s, c in all_prices if c != 'US']
        all_prices.insert(0, (us_price, us_service, us_country))
        logger.debug(f"📌 Priorizado US {us_service} a ${us_price} (precio competitivo)")

    logger.debug(f"📊 Ranking de precios combinado (top 10): {all_prices[:10]}")
    ...
    if force_service and force_country:
        ...
        if force_service == 'hero':
            full_order = HERO_COUNTRY_ORDER
        elif force_service == '5sim':
            full_order = list(fivesim_prices.keys()) if fivesim_prices else FIVESIM_MANUAL_ORDER
        elif force_service == 'grizzly':
            full_order = list(grizzly_prices.keys()) if grizzly_prices else GRIZZLY_COUNTRY_ORDER
        else:
        ...
        for purchase_country in countries_to_try:
            ...
            try:
                if force_service == 'grizzly':
                    result = await target_service['get_number'](purchase_country, service='am')
                    if result:
    ...
    if force_service and not force_country:
        ...
        if force_service == 'grizzly':
            country_order = list(grizzly_prices.keys()) if grizzly_prices else GRIZZLY_COUNTRY_ORDER
        elif force_service == 'hero':
            country_order = HERO_COUNTRY_ORDER
        elif force_service == '5sim':
            country_order = list(fivesim_prices.keys()) if fivesim_prices else FIVESIM_MANUAL_ORDER
        else:
    ...
    # -------------------------------------------------------------------
    # Caso 3: Sin fuerza - elegir el servicio y país más baratos disponibles
    # -------------------------------------------------------------------
    for price, service_name, purchase_country in all_prices:
        logger.debug(f"   Probando {service_name} en {purchase_country} (precio: ${price})...")
        ...
        try:
            ...
            logger.warning(f"   Error con {service_name} en {purchase_country}: {e}")
            continue
...
async def create_amazon_account(country_code, add_address_flag=True, max_retries=None, max_internal_retries=10):
    ...
    for global_attempt in range(1, retries + 1):
        ...
        try:
            ...
            # ----- PASO 2: Configurar y probar proxy -----
            proxy_url = None
            if PROXY_HOST_PORT:
                if PROXY_PROTOCOL == 'socks5':
                    proxy_url = f"socks5://{PROXY_AUTH}@{PROXY_HOST_PORT}" if PROXY_AUTH else f"socks5://{PROXY_HOST_PORT}"
                else:
                    proxy_url = f"http://{PROXY_AUTH}@{PROXY_HOST_PORT}" if PROXY_AUTH else f"http://{PROXY_HOST_PORT}"
                session.proxies = {'http': proxy_url, 'https': proxy_url}
                logger.debug(f"   ✅ Proxy configurado: {PROXY_PROTOCOL}://{PROXY_HOST_PORT}")
            else:
                logger.warning("   ⚠️ No se configuró proxy")

            logger.debug("🔄 Probando proxy...")
            ok, ip = test_proxy(proxy_url)
            if not ok:
            ...
            if PROXY_HOST_PORT:
                # Playwright soporta socks5 directamente
                if PROXY_PROTOCOL == 'socks5':
                    server_url = f"socks5://{PROXY_HOST_PORT}"
                else:
                    server_url = f"http://{PROXY_HOST_PORT}"
                ...
                logger.debug(f"   🌐 Proxy Playwright: {PROXY_PROTOCOL}://{PROXY_HOST_PORT}")
            ...
            while internal_attempt < max_internal_retries and not registration_success:
                ...
                try:
                        ...
                        for attempt in range(1, max_attempts + 1):
                            ...
                            if service_id:
                                try:
                                    ...
                                    elif service_name == 'grizzly':
                                        await cancel_grizzly(service_id)
                                except Exception as e:
                    ...
                    elif current_service == 'grizzly':
                        grizzly_prices = await get_grizzly_prices()
                        if grizzly_prices:
                            countries_to_try = list(grizzly_prices.keys())
                        else:
                            countries_to_try = GRIZZLY_COUNTRY_ORDER
                    else:
...
def generate():
    ...
    # --- PROTECCIÓN POR API KEY (Opción A) ---
    client_api_key = request.headers.get('X-API-Key', '')
    if GENERATOR_API_KEY and client_api_key != GENERATOR_API_KEY:
        return jsonify({'success': False, 'error': 'Unauthorized - Invalid API key'}), 401
    # -----------------------------------------

    # Obtener el header de autorización del USUARIO (token JWT) - opcional, modo no-op
    auth_header = request.headers.get('Authorization', '')
    user_token = None
    if auth_header.startswith('Bearer '):
        user_token = auth_header[7:]   # extrae el token del usuario

    # Verificar API_KEY del SERVICIO por header separado (opcional, legacy)
    service_api_key = request.headers.get('X-Service-API-Key', '')
    if API_KEY and service_api_key != API_KEY:
        return jsonify({'success': False, 'error': 'No autorizado - API key de servicio inválida'}), 401
    ...
    # Verificar créditos (modo no-op - siempre permite)
    ok, msg, role = check_user_credits(user_token, 4)
    if not ok:
        return jsonify({'success': False, 'error': msg}), 402

    # Verificar interruptor global (modo no-op - siempre activo)
    ...
    try:
        ...
        # En modo no-op, no descontamos créditos reales
        if result['success'] and user_token:
            success, new_credits = deduct_credits(user_token, 4)
            if success:
...
def diagnostic():
    return jsonify({
        ...
        'config': {
            ...
            'generator_api_key_protected': bool(GENERATOR_API_KEY),
            'hero_sms': bool(HERO_SMS_API_KEY),
            'fivesim': bool(FIVESIM_API_KEY),
            'grizzly_sms': bool(GRIZZLY_SMS_API_KEY),
